...
export class Pokemon {
  ...
  type1: PokeType = PokeType.Unknown;
  ...
  static createPokemonFromTraits(
    statString: string,
    labelString: string
  ): Pokemon {
    let res = new Pokemon();
    ...
    res.type2 =
      typeString.indexOf('/') >= 0
        ? (typeString.split('/')[1].toLowerCase() as PokeType)
        : ('none' as PokeType);
  ...
  static validatePokemonJson(json: any): void {
    ...
    for (const [key, expectedType] of Object.entries(optionalFields)) {
      ...
      if (value !== undefined && typeof value !== expectedType) {
        throw new Error(
          `Optional field ${key} should be of type ${expectedType}`
        );
      }
    }
  ...
  get_revealed_learned_move_pairs(): learned_move[] {
    return this.learn_levels.map((e, i) => {
      return {
        level: e,
        move:
          i < this.learned_moves_revealed_idx || this.fully_revealed
  ...
  get_revealed_tm_move_pairs(): tm_move[] {
    return this.tms.map((e, i) => {
      return {
        tm: e,
        move:
          this.tm_indexes_learned.indexOf(i) >= 0 || this.fully_revealed
            ? this.tm_moves[i]
            : '???',
      } as tm_move;
    });
  ...
  get_evos_from(): string[] {
    return this.next_evos.map((e, i) =>
      this.next_evos_revealed.indexOf(i) >= 0 || this.fully_revealed
        ? e
        : 'unknown'
    );
  }
   ...
   */
  get_evos_to(): string[] {
    return this.prev_evos.map((e, i) =>
      this.prev_evos_revealed.indexOf(i) >= 0 || this.fully_revealed
        ? e
        : 'unknown'
    );
  }
  ...
  getMovesIfRevealed(): string[] {
    ...
    if (this.fully_revealed) {
      ...
      res = res.concat(
        this.learned_moves.slice(0, this.learned_moves_revealed_idx)
      );
